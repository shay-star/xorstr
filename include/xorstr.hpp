...
template <typename CharT, size_t N, uint64_t... Keys> struct xorstr {
    ...
    [[nodiscard]] inline const CharT *reveal() {
        ...
        for (size_t i = 0; i < block; i++) {
            const __m256i encrypted_data = _mm256_loadu_si256(
                reinterpret_cast<const __m256i *>(reinterpret_cast<const uint8_t *>(encrypted_blocks.data()) + i * 32));
...
// 确保每次调用的初始种子都不一样
#define XOR_STR(s) make_xorstr<COMPILETIME_SEED>(s).reveal()
